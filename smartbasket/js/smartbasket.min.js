    ...
    t = jQuery, a = {
        ...
        getProducts: function () {
            ...
            return r.click(function (r) {
                ...
                delete a[n], e.setProducts(a), t(this).parents("." + s.attr("class")).remove(),
                    e.getSmartBasketMinState(a, "updateSmartBasketMin"), e.commonResult(a, "updateCommonResult")
            }), r
            ...
            let r = t("<form>").attr({
                ...
                o = t("<div>").attr({ class: "smart-basket__product-name" }).text("Корзина"),
                ...
                d = t("<span>").attr({ class: "smart-basket__info-msgs" }).html("Цена за 1 товар").append(p),
            ...
            n.append(o).append(u).append(l).append(m).append(k), r.append(n);
            // убрал .append(i) 
            ...
            for (let n in s) {
                ...
                a.productQuantityWrapper || (b.attr({ disabled: "disabled" }), f.attr({ disabled: "disabled" }),
                    v.attr({ disabled: "disabled" })), m.append(f).append(v).append(b);
                ...
                for (let a in s[n]) {
                    if ("sbId" === a && (o.html(s[n][a]), l.val(s[n][a]),
                    ...
                    if ("sbName" === a && (c.append(`<span>${s[n][a]}</span>`),
                        p.val(s[n][a])),
                        "sbSize" === a && void 0 !== s[n][a] && (c.children("span").append(`<span class="smart-basket__product-size"> - ${s[n][a]}</span>`),
                            d.val(s[n][a])), "sbPrice" === a && (_.html(s[n][a]),
                                h.val(s[n][a])), "sbPriceCommon" === a) {
                        let t = +s[n][a];
                        y.val(+t.toFixed(2))
                    }
                ...
                f.click(function (a) {
                    ...
                    let r = t(this).data("sbId").toString(),
                        ...
                        i = +h.val(),
                        o = i * n;
                    n > 1 && (o = i * --n, k.val(n), e.updateBasket(s, r, n, o), e.getSmartBasketMinState(s, "updateSmartBasketMin"),
                        e.commonResult(s, "updateCommonResult")),
                        h.val(i), y.val(+o.toFixed(2)), v.val(n)
                }),
                    //счетчик удаление
                    b.click(function (a) {
                        a.preventDefault();
                        let r = t(this).data("sbId").toString(),
                            ...
                            o = i * n;
                        n >= 1 ?
                            (o = i * ++n, k.val(n), e.updateBasket(s, r, n, o), e.getSmartBasketMinState(s, "updateSmartBasketMin"),
                                e.commonResult(s, "updateCommonResult")) :
                            n = 1, h.val(i), y.val(+o.toFixed(2)), v.val(n)
                    }),
                    o.append(l), c.append(u), c.append(p), c.append(d), m.append(k), _.append(h), g.append(y),
                    r.append(i.append(c).append(_).append(m).append(g).append(M))
                // убрал id .append(o)
            }
            ...
            return e.getSmartBasketMinState(o), c.click(function () {
                ...
                {
                    let i = {};
                    i.sbId = n, i.sbImg = t(this).data("sbProductImg"), i.sbName = t(this).data("sbProductName"),
                        ...
                        i.sbPrice.toFixed(2), a.productQuantityWrapper ?
                            (i.sbPriceCommon = +t(this).attr("data-sb-product-price") * t(this).parents("." + a.productElement).find("." + s.attr("class")).val(), i.sbPriceCommon.toFixed(2)) :
                            (i.sbPriceCommon = +t(this).attr("data-sb-product-price") * +t(this).data("sbProductQuantity"), i.sbPriceCommon.toFixed(2)),
                        r[n] = i, e.setProducts(r), l.empty(), l.append(e.showProducts(r)),
            ...
            if (0 == +n ? o.fadeIn() : o.fadeOut(), r) t("." + l.attr("class")).css("display", "block"), setTimeout(() => {
                ...
                if (!s)
                    return d.html(`<span>Всего товаров: </span> ${n}`),
                        p.html(
                            `<span>Общая стоимость: </span>
                             <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`
                        ),
                        c.append(o).append(l).append(d).append(p), c;
            ...
            allPrice = i; // вся сумма
        }, getSmartBasketMinState: function (s, r) {
            let n = 0;
            for (let t in s)
                for (let a in s[t])
                    "sbQuantity" === a && (n += +s[t][a]);
            ...
            for (let o in t) {
                ...
                for (let a in t[o])
                    "sbQuantity" === a && (n += t[o][a]),
                        "sbPriceCommon" === a && (i += +t[o][a])
            }
        }, userForm: function () {
            ...
            let address = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                inputAddress = t("<input>").attr({
                    class: "smart-basket__user-input",
                    type: "text",
            ...
            let c = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                p = t("<input>").attr({
                    class: "smart-basket__user-input",
                    type: "email",
                    placeholder: "Введите e-mail (необязательно)",
                    name: "userEmail",
            ...
            if (c.append(p), a.agreement.isRequired) {
                ...
                r = a.agreement.isLink ? t("<a>").attr({
                    ...
                    //href: a.agreement.isLink,     вылючил ссылку
                    target: "_blank",
                    rel: "nofollow"
                }) : t("<span>").attr({ class: "smart-basket__link-agreement" }),
                    ...
                    e.click(function () {
                        s.is(":checked") ? (r.text("Я не принимаю условия пользовательского соглашения"),
                            ...
                            d.addClass("smart-basket__input-wrapper_error")) : (r.text("Я принимаю условия пользовательского соглашения"),
            ...
            r.append(n).append(o).append(address).append(c),
                a.agreement.isChecked && r.append(d), k.append(u).append(m), s.append(r).append(k), m.click(function (t) {
                    ...
                    if (a.nameIsRequired && a.telIsRequired && a.emailIsRequired && a.addressIsRequired) {
                        if ("" === l.val() || "" === i.val() || "" === p.val() || "" === inputAddress.val() || r(s))
                            return "" === l.val() && l.parent().addClass("smart-basket__input-wrapper_error"), // телефон
                                "" === i.val() && i.parent().addClass("smart-basket__input-wrapper_error"),  // имя 
                                "" === inputAddress.val() && inputAddress.parent().addClass("smart-basket__input-wrapper_error"),
                                "" === p.val() && p.parent().addClass("smart-basket__input-wrapper_error"), !1; // почта
                        if (allPrice < 800) {
                            alert('Минимальная сумма заказа 800 ₽')
                        } else {
                        ...
                        if ("" === i.val() || !("" !== l.val() && a.telIsRequired || "" !== p.val() && a.emailIsRequired) || r(s))
                            return "" === l.val() && a.telIsRequired && l.parent().addClass("smart-basket__input-wrapper_error"),
                                "" === p.val() && a.emailIsRequired && p.parent().addClass("smart-basket__input-wrapper_error"),
                                "" === inputAddress.val() && a.addressIsRequired && inputAddress.parent().addClass("smart-basket__input-wrapper_error"),
                                "" === i.val() && i.parent().addClass("smart-basket__input-wrapper_error"), !1;
                        if (allPrice < 800) {
                            alert('Минимальная сумма заказа 800 ₽')
                        } else {
                            alert('Заказ принят, ожидайте звонка')
            ...
            let b = a => {
                a.focus(function () {
                    ...
                    t(this).parent().removeClass("smart-basket__input-wrapper_focus"),
                        "" === this.value && t(this).parent().addClass("smart-basket__input-wrapper_error")
                }), a.parent().on("input", function () {
                    t(this).removeClass("smart-basket__input-wrapper_error")
                }), a.change(function () {
                    t(this).parent().removeClass("smart-basket__input-wrapper_error"),
                        t(this).parent().addClass("smart-basket__input-wrapper_focus")
                })
            };
            return b(i), b(inputAddress), a.telIsRequired && b(l), a.emailIsRequired && b(p), s
            ...
            t.ajax({
                ...
                complete: function (s) {
                    ...
                    e.commonResult(r, "", "submitEvent"),
                        localStorage.clear(),
                        "" !== a.yaCounter.counterID && "" !== a.yaCounter.targetID && ym(a.yaCounter.counterID,
                            "reachGoal", a.yaCounter.targetID), setTimeout(() => {
