	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (isset($_POST['userAddress'])) {
			if (empty($_POST['userAddress'])) {
				...
				$address = "<b>Адрес: </b>" . strip_tags($_POST['userAddress']) . "; ";
			}
		}
		...
		$bodyHeader = '<table border="0" cellpadding="0" cellspacing="0" style="border-bottom:1px; border-right:1px; border-color:#e2e2e2; border-style: solid; width:800px" width="800" align="center">
			<tr>
				<th colspan="3" 
						style="width: 400px; padding-top:15px; padding-bottom:15px; padding-right:15px;
								 padding-left:15px; text-align:center; border-top:1px; border-left:1px;
								 border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;">' . $name . $tel . $address . $finalPrice . '</th>
				<th colspan="4"
						 style="width: 400px; padding-top:15px; padding-bottom:15px; padding-right:15px;
								 padding-left:15px; text-align:center; border-top:1px; border-left:1px;
								 border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;">' . $email . $agreement . '</th>
			</tr>';
		...
		foreach ($_POST as $key => $value) {
			...
			if (is_array($value) || $value instanceof Traversable) {
				foreach ($value as $k => $v) {
					...
					if ($k == 'productImg') {
						// $productImg = '<img src="' . $v . '" width="100" height="100" alt="картинка блюда">';
						// $body .=
						// 	'
